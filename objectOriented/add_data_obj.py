...
class addData:
    ...
    def DBConnect(self,dbName = None):
        ...
        """
        conn = mysql.connect(host='localhost', user='user', password='user',
                             database=self.dbName, buffered=True)
    
        if conn.is_connected():
            db_info = conn.get_server_info()
            print("Connected to MySQL Server vesion ", db_info)
            cur = conn.cursor()
            #cur.execute("select database ();")
            #record = cur.fetchone()
            #print("You are connected to databse: ",record)
        return conn, cur
...
if __name__ == "__main__":  
     addData_obj = addData('tweets')
     addData_obj.createDB('tweets')
     addData_obj.createTables('tweets')
     df = pd.read_csv('clean.csv')
     addData_obj.insert_to_tweet_table(dbName='tweets', df=df, table_name='TweetInformation')
